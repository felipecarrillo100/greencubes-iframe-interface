...
const ZoomIcon = ({enabled = true}: { enabled?: boolean }) => (
    <svg
        ...
        style={{cursor: enabled ? "pointer" : "not-allowed"}}
    >
        <g fill="none" stroke={enabled ? "#fab005" : "#495057"} strokeWidth="2" strokeLinecap="round"
           strokeLinejoin="round">
            {/* Top Left Arrow */}
...
export const LayerTreeViewer: React.FC<LayerTreeViewerProps> = ({
                                                                    layerTree,
                                                                    iframe
                                                                }) => {
    return (
...
const TreeNode: React.FC<TreeNodeProps> = ({node, iframe}) => {
    ...
    const handleToggleVisibility = (e: React.MouseEvent) => {
        e.stopPropagation();
        onLayerVisibilityChange(node.id, !node.visible);
    };
            ...
            {isGroup && isExpanded && (
                <div style={{
                    ...
                    {(node as JSONLayerGroup).children.map((child) => (
                        <TreeNode key={child.id} node={child} iframe={iframe}/>
                    ))}
                </div>
            )}
        </div>
